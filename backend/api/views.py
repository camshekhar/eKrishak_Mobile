...
from .serializers import UserChangePasswordSerializer, UserLoginSerializer, UserProfileSerializer, UserRegistrationSerializer, CropSerializer, OrderSerializer
...
def userLogin(request):
    ...
    if (userLogSerializer.is_valid(raise_exception=True)):
        ...
        is_vendor = userLogSerializer.data.get('is_vendor')
        is_farmer = userLogSerializer.data.get('is_farmer')

        user = authenticate(email=email, password=password)
        if (user is not None):
            if(user.is_vendor == is_vendor and user.is_farmer == is_farmer):
                token = get_tokens_for_user(user)
                return Response({'token': token, 'msg': 'Login Successful.'}, status=status.HTTP_200_OK)
            else:
                return Response({'errors':{'non_field_errors':['User is not Valid!']}}, status=status.HTTP_404_NOT_FOUND)
        else:
...
def addCrop(request):
    ...
    if crop.is_valid():
        crop.save()
        return Response(crop.data, status=status.HTTP_201_CREATED)
    # print(crop.errors)
    return Response(crop.errors, status=status.HTTP_400_BAD_REQUEST)
...
def getCrops(request):
    ...
    # print(cropSerializer.data)
    return Response(cropSerializer.data)

@api_view(['POST'])
def cropSell(request):
...
#         feed.save()
#         # message = "Item Added to Cart"
#         return Response(feed.data, status=status.HTTP_201_CREATED)
#     # message = "Item Already in Cart"
